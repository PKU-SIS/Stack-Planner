...
async def evaluate_answer(semaphore, answer, key_points, model):
    tasks = [
        evaluate_single_key_point(semaphore, key_point, answer, model)
...
async def evaluate_folder_async(subfolder_name, model, path_to_reports, key_point_dir):
    ...
    query_ids = query_ids
    tasks = [
        evaluate_query(semaphore, qid, folder_path, key_point_dir, model)
...
if __name__ == "__main__":
    ...
    results = asyncio.run(
        evaluate_folder_async(
            args.subfolder, args.open_ai_model, path_to_reports, path_to_key_point
        )
    )
