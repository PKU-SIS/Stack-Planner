...
def get_eval_metrics(qid, system, is_custom=False):
    ...
    base_path = (
        f"/data/group_data/cx_group/deepsearch_benchmark/reports/{system}{suffix}/"
    )
    ...
    if (
        qid not in detailed
        or qid not in citation
        or qid not in kpr
        or qid not in citation_recall
    ):
        print(
            f"Query ID {qid} not found in one or more files for system {system}{suffix}."
        )
        return None
...
for metric in metrics:
    ...
    print(
        f"{metric:20s}: {agree_pct:.1f}% agree, κ = {kappa:.3f} | Win = {win_pct:.1f}%, Lose = {lose_pct:.1f}% | N = {total}"
    )
...
for a1, a2 in combinations(annotators, 2):
    shared_qids = [
        ...
        if a1 in human_labels[qid] and a2 in human_labels[qid]
    ]
    if not shared_qids:
        continue
