...
summary_csv = Path(
    "/data/group_data/cx_group/deepsearch_benchmark/human_labeling/annotator_summary.csv"
)
...
annotator_votes = defaultdict(
    lambda: defaultdict(dict)
)  # metric -> query -> annotator -> vote
...
for annotator in annotators:
    ...
    for query_folder in annotator_dir.iterdir():
        ...
        mapping = {
            k: v.replace("search-r1-custom", "search-r1_custom")
        ...
        if (query_folder / "A.txt").exists() and not (query_folder / "B.txt").exists():
            human_label = 1
        elif (query_folder / "B.txt").exists() and not (
            query_folder / "A.txt"
        ).exists():
            human_label = 0
        else:
