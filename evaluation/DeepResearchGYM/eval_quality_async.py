...
async def evaluate_answer(semaphore, question, answer, criteria, model):
    tasks = [
        evaluate_single_criterion(semaphore, c, question, answer, model)
        for c in criteria
    ]
...
async def evaluate_query(semaphore, query_id, folder_path, model):
    ...
    try:
        evaluations = await evaluate_answer(
            semaphore, question, answer, EVAL_CRITERIA, model
        )
        return query_id, {
            "scores": evaluations,
        }
...
if __name__ == "__main__":
    ...
    detailed_path = (
        ...
        / args.subfolder
        / f"evaluation_results_detailed_{args.open_ai_model}.json"
    )
