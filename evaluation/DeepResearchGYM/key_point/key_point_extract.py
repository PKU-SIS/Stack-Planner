...
async def extract_key_point_single_query(
    semaphore, result_path, qid, question, CluewebIDs, texts, model
):
    tasks = [
        extract_key_point(semaphore, question, CluewebID, text, model)
        for CluewebID, text in zip(CluewebIDs, texts)
    ]
    ...
    results = {"question": question, "key_points": key_points}
...
async def extract_all_queries(sampled_queries, result_path, model):
    ...
    for data in sampled_queries:
        ...
        tasks.append(
            extract_key_point_single_query(
                semaphore, result_path, id, question, CluewebIDs, texts, model
            )
        )
