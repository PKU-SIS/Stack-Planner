...
async def crawl_urls(urls):
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        return [
            r.markdown if r.success else f"**Error for {r.url}:** {r.error_message}"
            for r in results
        ]
...
async def evaluate_query(openai_semaphore, query_id, answer_path, model):
    ...
    for claim in claims_to_urls:
        ...
        try:
            res = await check_citation_quality(
                openai_semaphore, claim_text, clean_docs, model
            )
            label = res["support"]
...
if __name__ == "__main__":
    ...
    output_path = (
        ...
        / args.subfolder
        / f"evaluation_results_citation_{args.open_ai_model}.json"
    )
