...
class FactStructLLMWrapper:
    ...
    def generate_initial_outline(
        ...
        在生成大纲前，请先在内部完成以下步骤（不要输出）：

        1. 拆解用户查询，识别其中：
        - 明确要求的分析维度
        - 必须覆盖的对象（如公司/国家/模型/时期等）
        - 明确的比较维度（如横向对比/优劣评估/趋势判断）
        - 明确的输出要求（如评估、排序、预测、建模等）

        2. 将这些“必须回应的要点”映射为大纲结构节点。

        ⚠️ 大纲结构必须覆盖用户提出的所有关键要求。
        ⚠️ 不得遗漏明确提出的分析维度。
        ...
        2. 一级节点数量建议 3–5 个
        3. 一级节点应优先对应“任务核心维度”，而不是泛背景或常规行业结构
        4. 仅在必要时添加二级节点
        5. 避免百科全书式展开
        6. 不要添加用户未要求的背景、政策、宏观趋势等内容（除非问题本身要求）

        ---

        ### 三、结构设计原则

        - 如果问题包含多个“必须分析的维度”，应优先将这些维度转化为一级节点
        - 如果问题包含多个“分析对象”（如多家公司），结构应支持逐对象展开
        - 如果问题包含“横向比较”，结构应为比较预留明确空间
        - 如果问题包含“预测/评估/排序”，必须预留单独节点
    ...
    def batch_refine_outline(
        ...
        for i, (node, docs) in enumerate(node_doc_pairs, 1):
            ...
            ⚠️ 所有局部优化必须保持与原始用户查询的一致性。
            ⚠️ 不得因为新信息而改变研究的核心问题或整体结构方向。

            ## 核心原则（非常重要）

            ### 一、节点职责一致性原则（最高优先级）

            1. 每个节点在整体大纲中承担特定“职责”（例如：某个分析维度、某类对象、某个阶段等）。
            2. 局部优化时：
            - 只需确保该节点继续忠实履行其“原有职责”
            - 不要求该节点覆盖用户问题的所有维度
            3. 不得扩大或改变该节点的职责边界。
    ...
    def _inherit_mab_state_for_existing_nodes(
        ...
        def inherit_recursive(new_node: OutlineNode):
            ...
            # # 策略2：无法匹配，保持默认状态（pull_count=0, reward_history=[]）
            else:
