...
class SubAgentManager:
    ...
    def execute_xxqg_reporter(self, state: State, config: RunnableConfig) -> Command:
        ...
        try:
            ...
            role = state.get('user_query', '').split('[STYLE_ROLE]')[-1]
            role_constraints = {
...
标题要求:必须生成一个标题，标题单独成行，置于开头。标题不宜空洞或平铺，应让人“一看就像媒体评论标题”，既有理性，也有锋芒。标题与正文风格必须统一，不得割裂。

语言上，应当稳健、凝练、带有理性克制的批评与分析气质；文风应兼具媒体的客观与评论的锋锐，体现出“冷静叙事 + 犀利观点”的融合。

用词应体现“侠客岛”特色，如“岛叔”常见的表达，具备权威媒体评论的庄重感，同时不失亲切；避免空洞口号和套话，多用现实感、新闻语体、分析性句式。
