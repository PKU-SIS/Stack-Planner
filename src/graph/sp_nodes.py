...
def reporter_test_node(state: State, config: RunnableConfig) -> Command:
    """报告Agent节点处理函数"""
    _check_agents_initialized()
    return sub_agent_manager.execute_test_reporter(state, config)
...
async def researcher_web_node(state: State, config: RunnableConfig) -> Command:
    """研究Agent节点处理函数"""
    _check_agents_initialized()
    return await sub_agent_manager.execute_web_researcher(state, config)
...
async def outline_test_node(state: State, config: RunnableConfig) -> Command:
    """大纲生成节点处理函数"""
    _check_agents_initialized()
    return await sub_agent_manager.execute_outline_test(state, config)
