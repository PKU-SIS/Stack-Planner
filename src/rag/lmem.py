...
class LmemProvider(Retriever):
    """
    LmemProvider is a provider that uses RAGFlow to retrieve LTM records.
    """
    ...
    def __init__(self):
        api_url = os.getenv("LMEM_RAGFLOW_API_URL")
        if not api_url:
            raise ValueError("LMEM_RAGFLOW_API_URL is not set")
        self.api_url = api_url

        api_key = os.getenv("LMEM_RAGFLOW_API_KEY")
        if not api_key:
            raise ValueError("LMEM_RAGFLOW_API_KEY is not set")
        self.api_key = api_key

        page_size = os.getenv("LMEM_RAGFLOW_PAGE_SIZE")
        if page_size:
