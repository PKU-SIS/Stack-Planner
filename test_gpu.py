...
async def run_concurrent_test(concurrency: int, max_tokens: int = 256) -> BenchmarkResult:
    ...
    for metrics in metrics_list:
        if isinstance(metrics, Exception):
            ...
            result.speed_results.append(metrics.tokens_per_second)
            result.total_times.append(metrics.total_time)
            result.total_tokens_list.append(metrics.output_tokens if metrics.output_tokens > 0 else metrics.total_tokens)
            result.success_count += 1
        else:
            result.fail_count += 1
            result.errors.append(metrics.error or "Unknown error")
...
def run_concurrency_benchmark(max_concurrency: int = 10, step: int = 2, start: int = 1, max_tokens: int = 256) -> dict:
    """
    运行最大并发请求数测试
    """
    print(f"\n{'='*60}")
