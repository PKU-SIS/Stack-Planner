...
def run_streaming_request(prompt: str, verbose: bool = True) -> StreamingMetrics:
    ...
    try:
        ...
        if verbose:
            print(f"\n      --- 输出结束 ---", flush=True)
            print(f"      [完成] 总耗时 {metrics.total_time:.2f}s, 生成 {metrics.output_tokens} tokens (生成时间 {metrics.generation_time:.2f}s)", flush=True)
...
async def run_streaming_request_async(prompt: str, request_id: int = 0) -> StreamingMetrics:
    """
    异步执行单次流式请求（用于并发测试）
    """
    metrics = StreamingMetrics()
...
async def run_concurrent_test(concurrency: int) -> BenchmarkResult:
    ...
    tasks = [
        run_streaming_request_async(TEST_PROMPT, i)
...
def run_concurrency_benchmark(max_concurrency: int = 10, step: int = 2) -> dict:
    ...
    for concurrency in range(1, max_concurrency + 1, step):
        ...
        if result.success_count > 0:
            ...
            print(f"  成功: {result.success_count}/{concurrency} | "
...
def print_summary(ttft_result: BenchmarkResult, concurrency_results: Optional[dict] = None):
    """
    打印测试结果汇总
    """
    print(f"\n{'='*60}")
